...
public class Molecule {
	...
	public Molecule(){
		...
		functionalGroups=new int [11][12];
		for (int i=1; i<=11; i++){
			for (int j=1; j<=12; j++)
			{
	 ...
	 * @return
	 */
	public String Locations(int n){
		String S="";
		boolean alert=false;
		for (int i=1; i<=length; i++){
			for (int j=1; j<=functionalGroups[n-1][i-1]; j++){
	...
	public String Prefix (int n){
		...
		S+=Locations(n);
		S+="-";
		S+=CountConverter.get(Sum(n));
		S+=PreConverter.get(n);
		return S;
	}
